...
export class HomePage {
  ...
  ionViewDidLoad(){
    this.setEntradasFiltradas();
    this.searchControl.valueChanges.debounceTime(600)
      .subscribe(search => {
  ...
  setEntradasFiltradas(){
    if(this.searchText && this.searchText.trim() != ''){
      this.veiculos = this.filtraEntradas(this.searchText);
    } else {
      this.buscaEntradas();
    }
  }
  ...
  filtraEntradas(event: any) {
    if(this.veiculos != undefined){
      return this.veiculos.filter((item) => {
        return item.placa.toLowerCase().indexOf(event.toLowerCase()) > -1;
      });
    }
  }
